...
// FUNÇÃO LOCAL: DESENHAR CARTA DE UPGRADE
// ==========================================
var _desenhar_carta = function(_cx, _cy, _tecla, _titulo, _val_atual, _custo_txt, _custo_cor, _pode_comprar) {
    var _cw = 260; // Largura da carta
    var _ch = 110; // Altura da carta
    var _espessura = 3; // O quão grossa você quer a borda (pode mudar para 4, 5...)
    
    // ========================================================
    // 1. DESENHA A BORDA SÓLIDA PRIMEIRO (Totalmente preenchida)
    // ========================================================
    if (_pode_comprar) {
        var _glow = abs(sin(tempo_menu * 0.05)) * 0.4; 
        draw_set_alpha(0.6 + _glow);
        draw_set_color(make_color_rgb(255, 215, 0)); // Dourado
        // Usamos "false" para desenhar um bloco sólido
    ...
    // ========================================================
    // 2. DESENHA O FUNDO DA CARTA POR CIMA (Escondendo o miolo)
    // ========================================================
    draw_set_color(make_color_rgb(15, 15, 20)); // Fundo cinza escuro opaco
    // Desenhamos no tamanho exato da carta, cobrindo o bloco gigante de cima
    ...
    draw_text(_cx + _cw/2, _cy + 75, "Custo: " + string(_custo_txt));
...
draw_set_color(c_white);
draw_text(_gui_w / 2, 80, "SANTUÁRIO DE UPGRADES");
...
// ==========================================
if (menu_page == 0) {
    _desenhar_carta(_grid_start_x + (_space_x * 0), _grid_start_y + (_space_y * 0), "1", "+ Vida Máxima", global.max_hp, string(custo_vida) + " Madeira", c_orange, (global.wood >= custo_vida));
    _desenhar_carta(_grid_start_x + (_space_x * 1), _grid_start_y + (_space_y * 0), "2", "+ Dano Base", global.player_damage, string(custo_dano) + " Carvão", c_dkgray, (global.carvao >= custo_dano));
    _desenhar_carta(_grid_start_x + (_space_x * 2), _grid_start_y + (_space_y * 0), "3", "+ Vel. Minerar", global.mine_speed, string(custo_vel) + " Ferro", c_ltgray, (global.iron >= custo_vel));
    
    _desenhar_carta(_grid_start_x + (_space_x * 0), _grid_start_y + (_space_y * 1), "4", "+ Drop Extra", global.mine_yield, string(custo_yield) + " Ouro", c_yellow, (global.gold >= custo_yield));
    _desenhar_carta(_grid_start_x + (_space_x * 1), _grid_start_y + (_space_y * 1), "5", "+ Vel. Ataque", global.atk_speed, string(custo_atk_speed) + " Ouro", c_yellow, (global.gold >= custo_atk_speed));
    _desenhar_carta(_grid_start_x + (_space_x * 2), _grid_start_y + (_space_y * 1), "6", "+ Vel. Movimento", global.player_move_speed, string(custo_move_speed) + " Carvão", c_dkgray, (global.carvao >= custo_move_speed));
    
    _desenhar_carta(_grid_start_x + (_space_x * 0), _grid_start_y + (_space_y * 2), "7", "+ Área Ataque", global.atk_area, string(custo_atk_area) + " Ferro", c_ltgray, (global.iron >= custo_atk_area));
    _desenhar_carta(_grid_start_x + (_space_x * 1), _grid_start_y + (_space_y * 2), "8", "+ Chance Crítico", string(global.crit_chance) + "%", string(custo_critico) + " Almas", c_aqua, (global.souls >= custo_critico));
    _desenhar_carta(_grid_start_x + (_space_x * 2), _grid_start_y + (_space_y * 2), "9", "+ Dano Crítico", string(global.crit_dano) + "x", string(custo_crit_dano) + " Almas", c_aqua, (global.souls >= custo_crit_dano));
}
// ==========================================
...
// ==========================================
    ...
    _desenhar_carta(_grid_start_x + (_space_x * 0), _grid_start_y + (_space_y * 0), "1", _fada_txt, _fada_status, string(_fada_unlock_c) + " Almas", c_aqua, (global.souls >= _fada_unlock_c));
    
    if (global.fairy_unlocked == 1) {
        _desenhar_carta(_grid_start_x + (_space_x * 1), _grid_start_y + (_space_y * 0), "2", "+ Vel. Atk Fada", global.fairy_atk_speed, string(custo_fada_vel) + " Ouro", c_yellow, (global.gold >= custo_fada_vel && global.fairy_atk_speed > 10));
        _desenhar_carta(_grid_start_x + (_space_x * 2), _grid_start_y + (_space_y * 0), "3", "+ Range Fada", global.fairy_vision, string(custo_fada_range) + " Ferro", c_ltgray, (global.iron >= custo_fada_range));
    }
    ...
    _desenhar_carta(_grid_start_x + (_space_x * 0), _grid_start_y + (_space_y * 1), "4", _burg_txt, _burg_status, string(_burg_unlock_c) + _burg_recurso_txt, _burg_cor_c, (_burg_recurso >= _burg_unlock_c));
    
    if (global.burguer_unlocked == 1) {
        _desenhar_carta(_grid_start_x + (_space_x * 1), _grid_start_y + (_space_y * 1), "5", "+ Vel. Cura", global.burguer_heal_speed, string(custo_burguer_speed) + " Ferro", c_ltgray, (global.iron >= custo_burguer_speed && global.burguer_heal_speed > 30));
    }
}
...
draw_text(_gui_w / 2, _gui_h - 50, "Pressione [ESPAÇO] para Ressuscitar");
draw_set_alpha(1);
draw_set_halign(fa_left);
